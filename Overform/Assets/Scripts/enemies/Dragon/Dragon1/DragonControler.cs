...
{
    private CapsuleCollider2D colliderDragon;
    ...
    void Start()
    {
        colliderDragon = GetComponent<CapsuleCollider2D>();
    ...
    void Update()
    {
        ...
        sideSign = Math.Sign(transform.position.x - player.position.x);

        if (Math.Abs(sideSign) == 1.0f)
        {
